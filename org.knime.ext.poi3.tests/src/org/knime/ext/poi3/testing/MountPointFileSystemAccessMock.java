 ...
 * ---------------------------------------------------------------------
 */
package org.knime.ext.poi3.testing;
...
/**
 * Mock implementation of {@link MountPointFileSystemAccess} for testing purposes.
 *
 * <p>This mock can be enabled/disabled via the {@link #enabled} flag to control
 * when test mountpoints are available to the system.</p>
 *
...
public class MountPointFileSystemAccessMock implements MountPointFileSystemAccess {
    ...
    /**
     * Static flag to enable/disable the mock mountpoint in tests.
     * Set to true in test setup to make mountpoints available.
     */
    ...
    public List<String> getMountedIDs() {
        if (enabled) {
            return List.of("test-mountpoint");
        } else {
    ...
    public URL resolveKNIMEURL(final URL url) throws IOException {
        return null; // NOSONAR: not used in tests
    }
