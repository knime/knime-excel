...
class ValuesParameters implements NodeParameters {
    ...
    private enum FormulaErrorHandlingOption {
            ...
            @Label(value = "Missing value",
                description = "Insert a Missing value. Inserting a missing value is type innocent, but also unobtrusive.")
            MISSING
    }
    ...
    @Widget(title = "Replace empty strings with missing values",
        description = "If checked, empty strings (i.e. strings with only whitespaces) are replaced with missing values. "
            + "This option is also applied to formulas that evaluate to strings.")
    ...
    @Widget(title = "Reevaluate formulas in all sheets",
        description = "If checked, formulas are reevaluated and put into the created table instead of using the cached values. "
            + "This can cause errors when there are functions that are not implemented by the underlying Apache POI library. "
            + "Note: Files with xlsb format do not support this option. For xlsx and xlsm files, reevaluation requires "
            + "significantly more memory as the whole file needs to be kept in memory (xls files are anyway loaded completely into memory).")
    @ValueReference(ReevaluateFormulasRef.class)
    boolean m_reevaluateFormulas = false;
