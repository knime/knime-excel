...
class EncryptionParameters implements NodeParameters {
    ...
    @Widget(title = "File encryption",
        description = "Allows you to specify a password to decrypt files that have been protected with a password via Excel. "
            + "Note: only files with password protection can be read when a password is specified. "
    ...
    void saveToConfig(final ExcelMultiTableReadConfig config) {
        ...
        switch (m_encryptionMode) {
            case NO_ENCRYPTION:
                ...
                break;
            case PASSWORD:
                authModel.setValues(AuthenticationType.PWD, null, "", m_credentials.getPassword());
                break;
            default:
    ...
    public void validate() throws InvalidSettingsException {
        if (m_encryptionMode == EncryptionMode.PASSWORD) {
            if (m_credentials == null || m_credentials.getPassword() == null || m_credentials.getPassword().isEmpty()) {
                throw new InvalidSettingsException("Password must not be empty when password encryption is selected.");
            }
        }
    }
