...
/**
 * A {@link ColumnNameCreator} to create column names based on the index of the columns in the input file.
 *
 * @author Lars Schweikardt, KNIME GmbH, Konstanz
 */
final class IndexColumnNameCreator implements ColumnNameCreator {

    private static final String STATIC_PREFIX = "Col";
     ...
     */
    public IndexColumnNameCreator(final TableReadConfig<ExcelTableReaderConfig> config,
        final TypedReaderTableSpec<KNIMECellType> spec) {
        m_prefix =
            config.useColumnHeaderIdx() ? config.getReaderSpecificConfig().getEmptyColHeaderPrefix() : STATIC_PREFIX;
        m_uniqueNameGenerator = ExcelColNameUtils.createUniqueNameGenerator(spec);
    }

    @Override
    public String createColumnName(final int index) {
        final String columnName = m_prefix + index;
